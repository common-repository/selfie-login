function FaceApiClient(apiServer, apiKey, apiSecret)
{
    var _server = '';
    var _format = "json";
    ...
    this.namespaceCreate = function (namespace) {
        var method = "namespace/create";
        var params = {namespace: namespace};
        CallMethod(method, null, params);
        return true;
    };

    this.namespaceRemove = function (namespace) {
        var method = "namespace/remove";
        var params = {namespace: namespace};
        CallMethod(method, null, params);
        return true;
    };

    this.namespaceList = function (namespace) {
        var method = "namespace/list";
    ...
    function CallMethod(method, files, params, callback)
    {
        ...
        if (!isDefined(files) && 0) {//0- we never use it 
            ...
            var responseId = "faceApiClientResponse" + request;
            window[callbackName] = function (data) {
                document.getElementById(responseId).parentNode.removeChild(document.getElementById(responseId));
            ...
            script.setAttribute("id", responseId);
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4) {
                    ...
                    else throw "Invalid status returned from API server (" + xhr.status + ")";
                }
            };
